...
public class Triangulate {
	...
	*/
	...
	public int Triangulator ( int nv, ArrayList<XYZ> pxyz, ITRIANGLE v[] )
	{
		...
		*/
		...
		for (int i=1;i<nv;i++)
		{
			if (pxyz.get(i).x < xmin) xmin = pxyz.get(i).x;
			if (pxyz.get(i).x > xmax) xmax = pxyz.get(i).x;
			if (pxyz.get(i).y < ymin) ymin = pxyz.get(i).y;
			if (pxyz.get(i).y > ymax) ymax = pxyz.get(i).y;
		}
		dx = xmax - xmin;
		dy = ymax - ymin;
		dmax = (dx > dy) ? dx : dy;
		...
		/*
			...
			the triangle list.
		*/
		pxyz.get(nv).x = xmid - 2.0 * dmax;
		pxyz.get(nv).y = ymid - dmax;
		pxyz.get(nv).z = 0.0;
		pxyz.get(nv+1).x = xmid;
		pxyz.get(nv+1).y = ymid + 2.0 * dmax;
		pxyz.get(nv+1).z = 0.0;
		pxyz.get(nv+2).x = xmid + 2.0 * dmax;
		pxyz.get(nv+2).y = ymid - dmax;
		pxyz.get(nv+2).z = 0.0;
		v[0].p1 = nv;
		v[0].p2 = nv+1;
		v[0].p3 = nv+2;
		...
		for (int i=0;i<nv;i++) {
			...
			*/
			...
			for (int j=0;j<ntri;j++)
			{
				if (complete[j])
					continue;
				x1 = pxyz.get(v[j].p1).x;
				y1 = pxyz.get(v[j].p1).y;
				x2 = pxyz.get(v[j].p2).x;
				y2 = pxyz.get(v[j].p2).y;
	...
	public static void main (String[] args)
	{
		...
		int ntri = triang.Triangulator( nv, points, triangles );
